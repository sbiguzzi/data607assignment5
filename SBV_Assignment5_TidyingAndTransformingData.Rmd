...
<span style="color: #969696;">The task for this weeks assignment is to: 

1. <span style="color: #969696;">Create a .CSV file (or optionally, a MySQL database!) that includes all of the information above. You’re encouraged to use a “wide” structure similar to how the information appears above, so that you can practice tidying and transformations as described below.
2. <span style="color: #969696;">Read the information from your .CSV file into R, and use tidyr and dplyr as needed to tidy and transform your data.
3. <span style="color: #969696;">Perform analysis to compare the arrival delays for the two airlines.
4. <span style="color: #969696;">Your code should be in an R Markdown file, posted to rpubs.com, and should include narrative
...
# Part 2 -  Load csv file and tranform data
...
```
```{r, results='asis', fig.align='left', echo=FALSE}
kable(flightdata)
```

## Transforming data
We notice that the data isn't set up great for analysis. The first thing to do is rename all the columns for readability. Second, there is a full empty row that needs to be removed. Third, the company name is only available for one of the rows that belongs to the airline. This needs to be fixed. Fourth, we want to change the shape of the data from wide to long as it will be easier to analyze. And finally, we want to transpose the on time and delayed flight status to their own columns.

1. Renaming columns
```{r}
#Rename first two columns
flightdata <- rename(flightdata,c('Airline' = 'X.U.FEFF.', 'Flight_Status' = 'X'))
...
5. Transpose on-time and delayed
```{r}
...
Our final data frame ready for analysis should look like this:
```{r, results='asis', echo=FALSE}
kable(flightdata_long)
```
...
flightdata_long <- flightdata_long %>%
  group_by(Airline) %>%
  mutate(total_on_time = sum(On_Time)/(sum(Delayed)+sum(On_Time)))
```
...
plot2 <-
  ggplot(
    as.data.frame(
      distinct(flightdata_long,total_on_time)
    ),
    aes(y = total_on_time, x = Airline, fill=Airline)
  ) +
...
label2 <-
  geom_text(
    aes(
      label=paste(
        as.character(
          round(total_on_time*100, digits = 0)
        ),
        "%",
        sep=""
      ),
...
```{r, echo=FALSE}
flightdata_long <- flightdata_long %>%
  mutate(airline_on_time = On_Time/(On_Time+Delayed))
```
...
plot <-
  ggplot(
    flightdata_long,
    aes(fill=Airline, y=airline_on_time, x=Airport)
  ) +
  geom_bar(
    width = 0.6,
  ...
  ggtitle(
    "Fig 2: Percent of on time flights by city and company"
  ) + 
  scale_y_continuous(
    expand = expansion(mult = c(0,.059))
  )
...
flightdata_long <- flightdata_long %>%
  group_by(Airline) %>%
  mutate(outlier = ifelse(is_outlier((On_Time+Delayed)),(On_Time+Delayed),as.numeric(NA)))

#Create scatter plot
plot3 <- 
  ggplot(
    flightdata_long,
    aes(x=On_Time, y=On_Time+Delayed, color=Airline)
  ) +
  geom_point(size=5, show.legend = FALSE) +
  ylab("Total flights") +
  xlab("Total on time") +
  ggtitle("Fig 3: Total flights vs Total on time") +
  scale_y_continuous(
...
label3 <- geom_label_repel(
  ...
  aes(
    label=paste(
      Airport,
      paste(as.character(On_Time),"on time",sep=" "),
      sep=", "
    )
  ),
  show.legend=FALSE,
...
plot4 <-
  ggplot(
    flightdata_long,
    aes(x=Airline, y=(On_Time+Delayed),fill=Airline)
  ) +
  geom_boxplot(show.legend = FALSE) +
  ylab(element_blank()) +
  xlab("Airline") +
  ggtitle("Fig 4: Total flights box plot") +
  scale_y_continuous(
    expand = expansion(mult = c(0.047,0.01))
  )
...
The reason we see AM WEST have 89% on time total flights compared to ALASKA's 87% on time is because of these outliers. The recommendation here is to look deeper into the data than just base summaries as you may find that your initial conclusion might be skewed by outliers.
